...
describe('Sample Project Server: server.js', () => {
    ...
    process.env.SAMPLE_PROJECT_DATABASE = `mongodb://${SOME_HOSTNAME}:${SOME_PORT}`
    ...
    test('databaseConnectCallback', () => {
        ...
        expect(mongodbState1).toEqual(`Yay - connected to the Sample Project database! mongodb://${SOME_HOSTNAME}:${SOME_PORT}`)
        expect(connectMock).toBeCalledWith(expect.any(Function))

        const mongodbState2 = databaseConnectCallback(SOME_ERROR)
        expect(mongodbState2).toEqual(`Bummer - unable to connected to the Sample Project database: mongodb://` +
    ...
    test('getHandler', () => {
        ...
        expect(response.send).toBeCalledWith(
            {
                'data': `${version} </br></br> ${'Bummer - unable to connected to the Sample Project database: ' + mongoURL + ', Connect Error: error'}</br></br> Echo Service Error: ${SOME_ERROR.message}`
            })
        ...
        expect(response.send).toBeCalledWith({data: `${version} </br></br>${'Bummer - unable to connected to the Sample Project database: ' + mongoURL + ', Connect Error: error'} </br></br> Echo Service Response: ${
                data.replace(/[\n\r]/g,'</br>')
            }`})
    ...
    test('serviceHandler', () => {
        ...
        expect(console.log).toHaveBeenNthCalledWith(1,
            `Bummer - unable to connected to the Sample Project database: mongodb://${SOME_HOSTNAME}:${SOME_PORT}`)
